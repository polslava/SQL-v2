create or replace FUNCTION     Getpredefinedtimeto (
...
--  END IF;
 ...
 /*переход с 01.04.2024 на новую сменность*/
  ...
  if v_ThisShift=1 then
     ...
     v_PureCurDate:= (v_PureCurDate) + 4/3;
   end if;
else
   if v_ThisShift=1 
    then
     v_PureCurDate:= (v_PureCurDate) + 8/24; --20:00 смена 1
   else
     v_PureCurDate:= (v_PureCurDate) + 20/24; --08:00 смена 2
   end if; 
end if;
